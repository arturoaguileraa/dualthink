    ...
    useEffect(() => {
        const fetchVideos = async () => {
            const res = await fetch(
                `https://api.pexels.com/videos/search?query=fashion&orientation=portrait&per_page=5&page=${page}`,
                {
            ...
            {videos.map((video, index) => (
                ...
                >
                    ...
                    <div className="absolute right-4 top-2/3 transform -translate-y-1/2 flex flex-col items-center gap-3">
                        ...
                        <div className="flex items-center flex-col gap-1">
                            <button
                                ...
                                className={`text-3xl p-3 rounded-full border-2 ${likes[video.id] ? 'border-red-500' : 'border-white/70'
                                    } bg-white/10 backdrop-blur-md`}
                            >
                                <FaHeart className={likes[video.id] ? 'text-red-500' : 'text-white'} />
                            </button>
                            <p className="text-white text-sm">{likeCounts[video.id]?.toLocaleString()}</p>
                        </div>
                        ...
                        <div className="flex items-center flex-col gap-1">
                            <button
                                ...
                                className="p-3 rounded-full border-2 border-white/70 bg-white/10 backdrop-blur-md text-white flex items-center justify-center"
                            >
                                <AiIcon />
                            </button>
                            <p className="text-white text-sm">Probar</p>
                        </div>
                        ...
                        <div className="flex items-center flex-col gap-1">
                            ...
                            <p className="text-white text-sm">Compartir</p>
                        </div>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 text-white p-4 bg-gradient-to-t from-black/60 to-transparent">
                        ...
                        <p className="text-base mb-3">Descubre esta prenda ideal para primavera-verano ☀️</p>
                    </div>
                </div>
            ))}
